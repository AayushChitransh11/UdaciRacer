...
function handleAccelerate() {
	...
	accelerate(raceId)
		.then(() => {
			console.log(`Race with ID ${raceId} is accelerating.`);
		})
		.catch((error) => {
			console.error("Error during race acceleration:", error);
		});
}
...
async function getTracks() {
	...
	try{
		...
		console.log("Tracks retrieved:",tracks);
		return tracks;
	}catch(error){
...
async function getRacers() {
	...
	try{
		...
		if(!response.ok){
			throw new Error(`Failed to fetch racers: ${response.statusText}`);
		}
		const racers=await response.json();
		console.log("Racers retrieved: ",racers);
		return racers;
	}catch(error){
		console.error("Error in getRacers:",error);
		throw error;
	}
...
async function getRace(id) {
	...
	try{
		...
		if(!response.ok){
			throw new Error(`Failed to fetch race with ID ${id}: ${response.statusText}`);
		}
		const race=await response.json();
		console.log("Race retrieved: ",race);
		return race;
	}catch(error){
...
async function accelerate(id) {
	...
	try {
		...
		if (!response.ok) {
			throw new Error(`Failed to accelerate race with ID ${id}: ${response.statusText}`);
		}
		const data = await response.json();
		console.log("Race acceleration data:", data);
	} catch (error) {
		console.error("Error in accelerate:", error);
	}
}